...
def recommend_workouts_from_data(bmi_category, age_category, workout_patterns, workout_db, age):
    ...
    if key in workout_patterns:
        ...
        if len(recommended_workouts) < recommended_count:
            fallback = recommend_workouts_fallback(bmi_category, workout_db, age)
        ...
        return recommend_workouts_fallback(bmi_category, workout_db, age)
...
def import_workout_db(file_path=None):
    try:
        ...
        print(f"Error importing workout database: {e}")
...
# Load data and workout database globally
# Check if running on Railway (using PORT environment variable as indicator)
if 'PORT' in os.environ:
    # Running on Railway, use relative paths
    csv_path = 'data_workout_final.csv'
    workout_db_path = 'workouts_data.py'
else:
...
def initialize_data():
    global df, workout_patterns, workout_db
    ...
    workout_db = import_workout_db(workout_db_path)
...
def recommend_workout():
    try:
        ...
        # Calculate BMI
        bmi_category, bmi_value = calculate_bmi_category(weight, height)
        
        # Get age category 
        age_category = get_age_category(age)
        
        # Get difficulty level
        difficulty_level = get_difficulty_level(bmi_category, age)
        ...
        if df is not None:
            workout_recommendations = recommend_workouts_from_data(bmi_category, age_category, workout_patterns, workout_db, age)
        else:
            workout_recommendations = recommend_workouts_fallback(bmi_category, workout_db, age)
...
def index():
    ...
    <body>
        ...
        <div class="endpoint">
            ...
            <p>Get personalized workout recommendations based on BMI and age</p>
            <h3>Request Body:</h3>
            <pre>
{
  "age": 30,
...
if __name__ == '__main__':
    # Initialize data
    data_available = initialize_data()
    
    # Print status
    if data_available:
        print("Data loaded successfully. API using data-driven recommendations where available.")
    else:
        print("Data not available. API using rule-based recommendations only.")
